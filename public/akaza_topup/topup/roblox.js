...
function renderProducts() {
    const robuxGrid = document.getElementById('robuxItems');
    if (robuxGrid) {
        robuxGrid.innerHTML = '';
        instantItems.forEach(item => createItemCard(item, robuxGrid, "🪙"));
    }
}
  ...
  if (uid.length > 2) {
    ...
    .then(data => {
      if (data.status === 'success') {
        ...
        checkedUsername = uid;
        usernameResult.className = 'username-result';
        usernameText.textContent = 'Username: ' + uid;
      }
    })
    .catch(err => {
      checkedUsername = uid;
