...
$classes = ($active ?? false)
            ? 'flex items-center px-4 py-3 text-sm font-bold text-white bg-gradient-to-r from-blue-600 to-blue-500 rounded-xl shadow-lg shadow-blue-500/30 transition-all duration-300 scale-[1.02]'
            : 'flex items-center px-4 py-3 text-sm font-medium text-gray-500 dark:text-gray-400 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700/30 hover:text-blue-600 dark:hover:text-blue-400 transition-all duration-300';
@endphp
...
<a {{ $attributes->merge(['class' => $classes]) }}>
    ...
    @elseif($icon == 'receipt')
        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h10a2 2 0 012 2v14a2 2 0 01-2 2z"/></svg>
    @elseif($icon == 'gamepad')
