        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 p-8 shadow-sm sm:rounded-lg border border-gray-700">
                <form action="{{ route('games.update', $game->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-300">Ganti Thumbnail Gambar (Opsional)</label>
                        @if($game->thumbnail)
                            <div class="mb-2">
                                <p class="text-xs text-gray-500 mb-1">Thumbnail Saat Ini:</p>
                                <img src="{{ str_starts_with($game->thumbnail, 'http') ? $game->thumbnail : asset('images/games/' . $game->thumbnail) }}" class="h-20 w-auto rounded border border-gray-600">
                            </div>
                        @endif
