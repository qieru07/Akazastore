    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            if (game && game.products) {
                membershipItems = game.products.filter(p => p.category === 'Membership');
                instantItems = game.products.filter(p => p.category !== 'Membership');
...
function renderProducts() {
    const membershipGrid = document.getElementById('membershipItems');
    const instantGrid = document.getElementById('instantItems');
    
    if (membershipGrid) {
        membershipGrid.innerHTML = '';
        membershipItems.forEach(item => createItemCard(item, membershipGrid, "🎫"));
    }
    
    if (instantGrid) {
        instantGrid.innerHTML = '';
        instantItems.forEach(item => createItemCard(item, instantGrid, "💎"));
    }
}
...
function checkUsername() {
  ...
  if (uid.length > 5) {
    ...
    .catch(err => {
      checkedUsername = '-';
      usernameResult.className = 'username-result';
      checkedUsername = 'FF_Player_' + uid.substring(0, 4);
